...
int main()
{
	...
	for(int i = 1; i <= 59; i++)
	{
		cout << i << endl;
		char name[50];
		sprintf(name, "CS585-CellImages/Normalized/%04d.jpg", i);
		src = imread(name, 0);

		Mat dst;
		segmentaion(src, dst);
		//adaptiveThreshold(src, dst, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 121, -7);
...
void segmentaion(Mat &src, Mat &dst)
{
	...
	adaptiveThreshold(img, thres_output, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 121, -4);
  ...
  //d= 3~4
	for (int i = 0; i<4; i++) dilate( thres_output, thres_output, dilate_elem );
	for (int i = 0; i<5; i++) erode(thres_output, thres_output, erosion_elem);
